...
# matchit (split data into training and test sets) ----
library(MatchIt)
library(caTools)
...
# GLM logistic regression with path 0 and 1 ---- 
# the coefficients gives the cahnge in the log odds of the outcome for a one unit INCREASE in the predictor variable
# Age - For every 1 unit change in Age, the log odds of Path increases by 0.03
# OR for a one unit increase in Age, the odds of bieng HGL/SCC is increased by a factor of 1.03
# Smoking - having been an ever smoker, versus never smoked, changes the log odds of HGL/SCC by -0.26
...
lesion <- docbrvs12 %>% 
  ...
  select(PathGrp3, SiteGrp, SmokingHx, LsWLGrp2, LesionLength, LesionWidth)


lesion %>% tbl_summary(by = "LsWLGrp2", statistic = list(all_categorical()~"{n} ({p}%)",
                                                         LesionLength ~ "{mean} ({sd})", 
                                                         LesionWidth ~ "{mean} ({sd})"),
                       label = list(PathGrp3 ~ "Pathology", SiteGrp ~ "Lesion site")) %>%
  add_p(test = list(all_continuous()~"anova", all_categorical()~"chisq.test")) %>%
  add_overall()

# table of pathological variables ----
lesion <- docbrvs12 %>% 
  mutate(SiteGrp = ifelse(SiteRisk %in% c("1", "2"), "BM/GING/HP/SP", "TONG/FOM")) %>%
  mutate(PathGrp3 = ifelse(Pathgrp %in% c("0", "0.5", "1"), "Normal/Reactive/LGL", "HGL/SCC")) %>%
  mutate(PathGrp3 = factor(PathGrp3, levels = c("Normal/Reactive/LGL", "HGL/SCC"))) %>% filter(!is.na(Path)) %>% droplevels() %>%
  select(PathGrp3, Age, SmokingHx, SiteGrp, LsWLGrp2, LesionLength, LesionWidth)

lesion %>% 
  tbl_summary(by = "PathGrp3", statistic = list(all_categorical()~"{n} ({p}%)",
                                                LesionLength ~ "{mean} ({sd})", 
                                                LesionWidth ~ "{mean} ({sd})"),
              SiteGrp ~ "Lesion site") %>%
...
# table of training vs test vs valid ----
names(docbrvs12)
...
rocsets <- docbrvs12 %>% 
  ...
  mutate(PathGrp3 = factor(PathGrp3, levels = c("Normal control", "Reactive/LGL", "HGL/SCC"))) %>%
  select(PathGrp3, SiteGrp, SmokingHx, LsWLGrp2, newSet5) %>% filter(!is.na(PathGrp3)) %>% droplevels()
...
# dot plot of lesion size and cellularity ----
names(doc_morphvs)
...
# Histo plot DNA_Index histogram by group ----
